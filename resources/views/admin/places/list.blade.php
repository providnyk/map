...
@section('title'){!! trans('app/places.list.title') !!}@endsection
    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-item active">{!! trans('app/places.breadcrumbs.list') !!}</span>
            </div>
            <a href="{!! route('admin.places') !!}" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
    </div>
@endsection
...
@section('script')
    <script>
        $(document).ready(function(){
            ...
            let dt = $('.table').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        sortable: false,
                        data: function(row){
                            return `<a href="{!! route('admin.places.form', [':id']) !!}" class="btn btn-sm btn-primary"><i class="icon-pencil"></i></a>`.replace(':id', row.id);
                        }
                    }
                ],
                ajax: {
            ...
            $('#btn-add').on('click', (e) => {
                window.location.href = '{!! route("admin.places.form") !!}';
            });
    ...
    <div class="card">
        <div class="card-body p-0">
            <div class="container-fluid">
                <div class="row filters px-4 pt-3">
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="name" data-filter-type="text" data-default-value="">
                        <label>{!! trans('app/places.list.filters.name') !!}</label>
                        <input type="text" class="form-control input-sm" placeholder="{!! trans('app/places.list.filters.name') !!}">
                    </div>

                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="cities" data-filter-type="select" data-default-value="">
                        <label>{!! trans('app/places.list.filters.city') !!}</label>
                        <div>
                            <select name="cities" class="form-control multi-select" data-placeholder="{!! trans('app/places.list.filters.city') !!}" multiple>
            ...
            <table class="table table-bordered table-striped table-styled">
                <thead>
                    <tr>
                        <th width="1px">{!! trans('app/places.list.table.columns.id') !!}</th>
                        <th width="25%">{!! trans('app/places.list.table.columns.name') !!}</th>
                        <th width="25%">{!! trans('app/places.list.table.columns.city') !!}</th>
                        <th width="25%">{!! trans('app/places.list.table.columns.created_at') !!}</th>
                        <th width="25%">{!! trans('app/places.list.table.columns.updated_at') !!}</th>
                        <th width="1px">{!! trans('app/places.list.table.columns.actions') !!}</th>
                    </tr>
                </thead>
            </table>
        </div>
