...
@section('title'){!! trans('app/galleries.list.title') !!}@endsection
    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-item active">{!! trans('app/galleries.breadcrumbs.list') !!}</span>
            </div>
            <a href="{!! route('admin.galleries') !!}" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function(){
            ...
            let dt = $('.table').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        sortable: false,
                        data: function(row){
                            return `<a href="{!! route('admin.galleries.form', [':id']) !!}" class="btn btn-sm btn-primary"><i class="icon-pencil"></i></a>`.replace(':id', row.id);
                        }
                    }
                ],
                ajax: {
                    url: '{!! route('api.galleries.index') !!}',
                    data: function(data){
            ...
            function deleteEntries(){
                ...
                $.ajax({
                    type: 'post',
                    url: '{!! route('api.galleries.delete') !!}',
                    data: {
                        'ids': ids
                    },
            ...
            $('#btn-add').on('click', (e) => {
                window.location.href = '{!! route("admin.galleries.form") !!}';
            });
    ...
    <div class="card">
        <div class="card-body p-0">
            ...
            <table class="table table-bordered table-striped table-styled">
                <thead>
                    <tr>
                        <th width="1px">{!! trans('app/galleries.list.table.columns.id') !!}</th>
                        <th width="33%">{!! trans('app/galleries.list.table.columns.name') !!}</th>
                        <th width="33%">{!! trans('app/galleries.list.table.columns.created_at') !!}</th>
                        <th width="33%">{!! trans('app/galleries.list.table.columns.updated_at') !!}</th>
                        <th width="1px">{!! trans('app/galleries.list.table.columns.actions') !!}</th>
                    </tr>
                </thead>
            </table>
        </div>
