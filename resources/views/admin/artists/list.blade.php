...
@section('title'){!! trans('app/artists.list.title') !!}@endsection
    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-item active">{!! trans('app/artists.breadcrumbs.list') !!}</span>
            </div>
            <a href="{!! route('admin.artists') !!}" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
    </div>
@endsection
...
@section('script')
    <script>
        $(document).ready(function(){
            ...
            let dt = $('.table').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        sortable: false,
                        data: function(row){
                            return `<a href="{!! route('admin.artists.form', [':id']) !!}" class="btn btn-sm btn-primary"><i class="icon-pencil"></i></a>`.replace(':id', row.id);
                        }
                    }
                ],
                ajax: {
            ...
            $('#btn-add').on('click', (e) => {
                window.location.href = '{!! route("admin.artists.form") !!}';
            });
    ...
    <div class="card">
        <div class="card-body p-0">
            <div class="container-fluid">
                <div class="row filters px-4 pt-3">
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="name" data-filter-type="text" data-default-value="">
                        <label>{!! trans('app/artists.list.filters.name') !!}</label>
                        <input type="text" class="form-control input-sm" placeholder="{!! trans('app/artists.list.filters.name') !!}">
                    </div>
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="email" data-filter-type="text" data-default-value="">
                        <label>{!! trans('app/artists.list.filters.email') !!}</label>
                        <input type="text" class="form-control input-sm" placeholder="{!! trans('app/artists.list.filters.email') !!}">
                    </div>

					<div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="festival" data-filter-type="select" data-default-value="">
						<label>{!! trans('app/artists.list.filters.festival') !!}</label>
						<div>
							<select class="form-control" data-placeholder="{!! trans('app/artists.list.filters.festival') !!}" multiple>
							@foreach($festivals as $festival)
								<option value="{!! $festival->id !!}">[{!! $festival->year !!}] {!! $festival->name !!}</option>
							@endforeach
							</select>
						</div>
					</div>

                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="member" data-filter-type="select" data-default-value="">
                        <label>{!! trans('app/artists.list.filters.member') !!}</label>
                        <div>
                            <select class="form-control" data-placeholder="{!! trans('app/artists.list.filters.member') !!}" multiple>
                                <option value="team">{!! trans('app/artists.form.fields.team_member.label') !!}</option>
                                <option value="board">{!! trans('app/artists.form.fields.board_member.label') !!}</option>
                            </select>
                        </div>
                    </div>
					<div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="vocation" data-filter-type="select" data-default-value="">
						<label>{!! trans('app/artists.list.filters.vocation') !!}</label>
						<div>
							<select class="form-control" data-placeholder="{!! trans('app/artists.list.filters.vocation') !!}" multiple>
                    ...
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="created_at" data-filter-type="date-range" data-default-value="{!! $dates['min_created_at'] . '|' . $dates['max_created_at'] !!}">
                        <label>{!! trans('app/artists.list.filters.created_at') !!}</label>
                        <div class="form-group form-group-feedback form-group-feedback-left">
                            <input class="form-control input-sm date-range" placeholder="{!! trans('app/artists.list.filters.created_at') !!}">
                            <div class="form-control-feedback form-control-feedback-lg"><i class="icon-calendar2"></i></div>
                        </div>
                    </div>
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="updated_at" data-filter-type="date-range" data-default-value="{!! $dates['min_created_at'] . '|' . $dates['max_created_at'] !!}">
                        <label>{!! trans('app/artists.list.filters.updated_at') !!}</label>
                        <div class="form-group form-group-feedback form-group-feedback-left">
                            <input class="form-control input-sm date-range" placeholder="{!! trans('app/artists.list.filters.updated_at') !!}">
                            <div class="form-control-feedback form-control-feedback-lg"><i class="icon-calendar2"></i></div>
                        </div>
                    </div>
                </div>
            ...
            <table class="table table-bordered table-striped table-styled">
                <thead>
                <tr>
                    <th width="10%">{!! trans('app/artists.list.table.columns.image') !!}</th>
                    <th width="22%">{!! trans('app/artists.list.table.columns.name') !!}</th>
                    <th width="22%">{!! trans('app/artists.list.table.columns.profession') !!}</th>
                    {{--<th width="15%">{!! trans('app/artists.list.table.columns.team_member') !!}</th>--}}
                    {{--<th width="15x">{!! trans('app/artists.list.table.columns.board_member') !!}</th>--}}
                    <th width="22%">{!! trans('app/artists.list.table.columns.festivals') !!}</th>
                    <th width="22%">{!! trans('app/partners.list.table.columns.created_at') !!}</th>
                    <th width="1px">{!! trans('app/artists.list.table.columns.actions') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
