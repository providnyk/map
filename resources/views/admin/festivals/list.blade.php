...
@section('title'){!! trans('app/festivals.list.title') !!}@endsection
    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-item active">{!! trans('app/festivals.breadcrumbs.list') !!}</span>
            </div>
            <a href="{!! route('admin.festivals') !!}" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
    </div>
@endsection
        ...
        $(document).ready(function(){
            ...
            let dt = $('.table').DataTable({
                ...
                columns: [
                    ...
                    {
                        render: function(data, type, row){
                            return `<a href="{!! route('admin.festivals.form', [':id']) !!}" class="btn btn-sm btn-primary"><i class="icon-pencil"></i></a>`.replace(':id', row.id);
                        }
                    }
                ],
                ajax: {
            ...
            function deleteEntries(){
                ...
                $.ajax({
                    type: 'post',
                    url: '{!! route('api.festivals.delete') !!}',
                    data: {
                        'ids': ids
                    },
            ...
            $('#btn-add').on('click', (e) => {
                window.location.href = '{!! route("admin.festivals.form") !!}';
            });
    ...
    <div class="card">
        <div class="card-body p-0">
            <div class="container-fluid">
                <div class="row filters px-4 pt-3">
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="name" data-filter-type="text" data-default-value="">
                        <label>{!! trans('app/festivals.list.filters.name') !!}</label>
                        <input type="text" class="form-control input-sm" placeholder="Enter name">
                    </div>
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="active" data-filter-type="select" data-default-value="*">
                        <label>{!! trans('app/festivals.list.filters.active') !!}</label>
                        <div>
                            <select class="form-control" data-placeholder="Status">
                                <option value="*">{!! trans('app/common.all') !!}</option>
                                <option value="1">{!! trans('app/common.status.active') !!}</option>
                                <option value="0">{!! trans('app/common.status.inactive') !!}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="create_at" data-filter-type="date-range" data-default-value="{!! $dates['min_created_at'] . '|' . $dates['max_created_at'] !!}">
                        <label>{!! trans('app/festivals.list.filters.created_at') !!}</label>
                        <div class="form-group form-group-feedback form-group-feedback-left">
                            <input class="form-control input-sm date-range" placeholder="{!! trans('app/festivals.list.filters.created_at') !!}">
                    ...
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="updated_at" data-filter-type="date-range" data-default-value="{!! $dates['min_updated_at'] . '|' . $dates['max_updated_at'] !!}">
                        <label>{!! trans('app/festivals.list.filters.updated_at') !!}</label>
                        <div class="form-group form-group-feedback form-group-feedback-left">
                            <input class="form-control input-sm date-range" placeholder="{!! trans('app/festivals.list.filters.updated_at') !!}">
                            <div class="form-control-feedback form-control-feedback-lg"><i class="icon-calendar2"></i></div>
                        </div>
                    </div>
                </div>
            ...
            <table class="table table-bordered table-striped table-styled">
                <thead>
                <tr>
                    <th width="1px">{!! trans('app/festivals.list.table.columns.id') !!}</th>
                    <th width="25%">{!! trans('app/festivals.list.table.columns.name') !!}</th>
                    <th width="25%">{!! trans('app/festivals.list.table.columns.year') !!}</th>
                    <th width="25%">{!! trans('app/festivals.list.table.columns.status') !!}</th>
                    <th width="25%">{!! trans('app/festivals.list.table.columns.published') !!}</th>
                    <th width="1px">{!! trans('app/festivals.list.table.columns.actions') !!}</th>
                </tr>
                </thead>
            </table>
        </div>
