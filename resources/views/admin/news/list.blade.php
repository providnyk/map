...
@section('title'){!! trans('app/news.list.title') !!}@endsection
    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-item active">{!! trans('app/news.breadcrumbs.list') !!}</span>
            </div>
            <a href="{!! route('admin.news') !!}" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
        </div>
    </div>
@endsection
...
@section('script')
    <script>
        $(document).ready(function(){
            ...
            let dt = $('.table').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        //sortable: false,
                        data: function(row){
                            return `<a href="{!! route('admin.news.form', [':id']) !!}" class="btn btn-sm btn-primary"><i class="icon-pencil"></i></a>`.replace(':id', row.id);
                        }
                    }
                ],
                ajax: {
            ...
            $('#btn-add').on('click', (e) => {
                window.location.href = '{!! route("admin.news.form") !!}';
            });
    ...
    <div class="card">
        <div class="card-body p-0">
            <div class="container-fluid">
                <div class="row filters px-4 pt-3">
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="title" data-filter-type="text" data-default-value="">
                        <label>{!! trans('app/news.list.filters.title') !!}</label>
                        <input type="text" class="form-control input-sm" placeholder="{!! trans('app/news.list.filters.title') !!}">
                    </div>
                    @if($festivals)
                        <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="festivals" data-filter-type="select" data-default-value="">
                            <label>{!! trans('app/news.list.filters.festivals') !!}</label>
                            <div>
                                <select name="festivals[]" class="form-control multi-select" data-placeholder="{!! trans('app/news.list.filters.festivals') !!}" multiple>
                    ...
                    @if($categories)
                        <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="categories" data-filter-type="select" data-default-value="">
                            <label>{!! trans('app/news.list.filters.categories') !!}</label>
                            <div>
                                <select name="categories[]" class="form-control multi-select" data-placeholder="{!! trans('app/news.list.filters.categories') !!}" multiple>
                    ...
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="created_at" data-filter-type="date-range" data-default-value="{!! $dates['min_created_at'] . '|' . $dates['max_created_at'] !!}">
                        <label>{!! trans('app/places.list.filters.created_at') !!}</label>
                        <div class="form-group form-group-feedback form-group-feedback-left">
                            <input class="form-control input-sm date-range" placeholder="{!! trans('app/places.list.filters.created_at') !!}">
                    ...
                    <div class="filter col-md-12 col-lg-6 col-xl-4 py-2" data-name="updated_at" data-filter-type="date-range" data-default-value="{!! $dates['min_updated_at'] . '|' . $dates['max_updated_at'] !!}">
                        <label>{!! trans('app/places.list.filters.updated_at') !!}</label>
                        <div class="form-group form-group-feedback form-group-feedback-left">
                            <input class="form-control input-sm date-range" placeholder="{!! trans('app/places.list.filters.updated_at') !!}">
                            <div class="form-control-feedback form-control-feedback-lg"><i class="icon-calendar2"></i></div>
                        </div>
                    </div>
                </div>
            ...
            <table class="table table-bordered table-striped table-styled">
                <thead>
                    <tr>
                        <th width="1px">{!! trans('app/news.list.table.columns.id') !!}</th>
                        <th width="1px">{!! trans('app/news.list.table.columns.image') !!}</th>
                        <th width="25%">{!! trans('app/news.list.table.columns.title') !!}</th>
                        <th width="25%">{!! trans('app/news.list.table.columns.festival') !!}</th>
                        <th width="25%">{!! trans('app/news.list.table.columns.category') !!}</th>
                        <th width="25%">{!! trans('app/news.list.table.columns.published') !!}</th>
                        <th width="1px">{!! trans('app/news.list.table.columns.actions') !!}</th>
                    </tr>
                </thead>
            </table>
        </div>
