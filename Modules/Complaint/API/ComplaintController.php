...
class ComplaintController extends Controller
{
	...
	public function sendIt(Array $a_complaints, Bool $b_dry_run = NULL) : void
	{
		$s_date		= date("m/d/Y");
...
#		$s_name_from	= $a_from[0]['s_title'];
	 ...
	 * @return Response	json instance of
	 */
#	public function index(ComplaintApiRequest $request, ComplaintFilters $filters) : \Illuminate\Http\Response
	public function index(ComplaintRequest $request, ComplaintFilters $filters) : \Illuminate\Http\Response
	{
		...
		{
			$a_complaints = $a_content['data'][$i];
			if (isset($a_complaints['opinion'][0]))
				$a_complaints['latest_opinion'] = $a_complaints['opinion'][0]['title'];
			$a_rating = [];
			$a_rating = self::_sumAllVotes($a_complaints['vote'], $a_marks);
			$a_rating = self::_setAverageTotal($a_rating, $a_elements, $i_mark_max, $i_mark_min);
			$a_complaints['rating'] = $a_rating;

			unset($a_complaints['opinion']);
			unset($a_complaints['vote']);
			$a_content['data'][$i] = $a_complaints;

			if ($b_test && isset($a_complaints['rating']['overall']['percent']) && $a_complaints['rating']['overall']['percent'] < 35)
			{
				self::sendIt($a_complaints, $b_dev);
				$b_test  = FALSE;
			}
	 ...
	 *
	 * @param String	$s_format_set	Format describing complaintment of information
	 ...
	 * for each element and for the complaint in general
	 *
	 * @param Array		$a_rating		All votes organised per element and per complaint
					...
					$a_res['element'][] =
						[	'percent'		=> $i_percent,
							'description'	=> self::_setMarkDescription(
								...
								Complaint::formatNumber($i_tmp, 1),
								Complaint::formatNumber($i_sum_votes),
								]
							),
						]
							;
#					if ($i_tmp > $i_mark_min)
		...
		/**
		 * Total rating for the complaint
...
#			$i_tmp = $i_mark_max * ($i_overall / 100);
			...
			$a_res['overall']['description'] = self::_setMarkDescription(
						...
						Complaint::formatNumber($i_overall),
						]
					);
			$a_res['overall']['details'] = self::_setMarkDescription(
						...
						[
						Complaint::formatNumber($i_tmp, 1),
						Complaint::formatNumber($i_mark_max),
						Complaint::formatNumber($i_overall_votes),
						]
					);
		}
		return $a_res;
	 ...
	 *
	 * @param Array		$a_votes		All votes casted for this complaint
