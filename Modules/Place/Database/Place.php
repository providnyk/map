...
class Place extends Model
{
	 ...
	 * @param Integer	$i_debug		which place id debug email sendng
	 *
	 * @return void
	 */
	public static function calculateRating(Int $i_debug = 0) : void
	{
		...
		{
			$o_sql		= self::wherePublished(1)->where('rating_last', '<', Carbon::now()->subHour($i_update_freq));
		}
		$o_sql			= $o_sql
						->with(['opinion', 'vote'])
						;
	 ...
	 * @return void
	 */
	private static function _calculateRating(Object $o_sql, Int $i_debug = 0) : void
	{
		$i_lowest_allowed 	= 35;
		$i_emails_per_day	= 1;
		$i_batch_update		= 7;
		...
		for ($i = 0; $i < count($a_data); $i++)
		{
			...
			if ($b_send || $i_debug > 0)
			{
				...
				Complaint::sendIt($o_place, $s_tmp, ($i_debug > 0));
